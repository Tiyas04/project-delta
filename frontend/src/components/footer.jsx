    ...
    return (
        <footer style={{
            ...
            <div style={{
                ...
                <div>
                    {/* Logo */}
                    <div id="footer-logo" style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                        <div style={{
                    ...
                    <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.35)', lineHeight: 1.6, marginBottom: 20 }}>
                        The all-in-one platform for video calls, rooms, and real-time connections.
                    </p>
                </div>
                ...
                {Object.entries(footerCols).map(([col, links]) => (
                    <div key={col}>
                        ...
                        {links.map((link) => (
                            <a
                                key={link}
                                id={`footer-link-${col.toLowerCase()}-${link.toLowerCase().replace(/\s+/g, '-')}`}
                                href="#"
                ...
                <div>
                    ...
                    <div style={{ display: 'flex', gap: 10 }}>
                        {['Discord', 'Twitter', 'Instagram', 'LinkedIn'].map((s) => (
                            <div key={s} id={`footer-social-${s.toLowerCase()}`} className="social-btn">
                                <SocialIcon label={s} />
                            </div>
                        ))}
                    </div>
